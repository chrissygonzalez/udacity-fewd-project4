/**
* Project 4: Web/browser optimization
* Optimized by Chrissy Gonzalez as part of Udacity FEWD Nanodegree
* v2
*
* This file contains the code to generate random pizzas, add them to the screen,
* and place/move a number of background pizzas as the page is scrolled.
* It also powers a slider that sets foreground pizza size to small, medium, or large.
*/
...
var resizePizzas = function(size) {
  ...
  // Returns pizza size as set by slider. Called by changePizzaSlices(size). **Optimized by CG**
...
/**
* Optimizations:
* 1. getElementsByClassName instead of querySelectorAll
* 2. save reference to the pizzaContainers and pull out of loop
* 3. save reference to pizzaContainers.length for more efficient loop
* 4. delete determineDx function and use sizeSwitcher to set % size instead
*/
  function changePizzaSizes(size) {
    ...
    for (var i = 0; i < len; i++) {
      pizzaContainers[i].style.width = (sizeSwitcher(size) * 100) + '%'; // set pizza size as percentage
    }
  }
...
var pizzasDiv = document.getElementById("randomPizzas"); // Optimization: moved variable outside loop
for (var i = 2; i < 100; i++) {
...
function updatePositions() {
  ...
  var items = document.getElementsByClassName('mover');       // Optimization: getElementsByClassName instead of querySelectorAll
  var top = (document.body.scrollTop / 1250);                 // Optimization: pull the scrollTop calc outside of loop and save as variable
  var phase, len = items.length;                              // Optimization: save phase and array length outside of loop

  for (var i = 0; i < len; i++) {
    phase = Math.sin(top + i % 5) * 100 + 'px';               // Optimization: set phase to all the previous calculations
    items[i].style.transform = "translateX(" + phase + ")";   // Optimization: translateX instead of left property to avoid layout
  }
...
/**
* Optimization: throttle scroll event using example at https://developer.mozilla.org/en-US/docs/Web/Events/scroll
*/
 (function() {
...
/**
* Optimizations: reduce # of pizzas, resize pizza image,
* remove height/width styles, save values outside of loop
*/
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 300;
  var elem; // Optimization: save outside loop
  var movingPizzas = document.getElementById('movingPizzas1'); // Optimizations: save outside of loop, updated to getElementById
  for (var i = 0; i < 24; i++) {
    elem = document.createElement('img');
    elem.className = 'mover';
    elem.src = "images/pizza_100.png";
    elem.style.left = (i % cols) * s + 'px'; // Optimization: remove basicLeft and use left instead
